...
function EmployeeProfile({ employee }) {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <div className="flex items-start gap-5">
          <img
            src={emp.avatar}
            alt={emp.name}
            className="w-16 h-16 rounded-full bg-slate-100 flex-shrink-0"
          />
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-gray-600 mt-0.5">{emp.role}</p>
      ...
      {/* ── Навыки ── */}
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <h2 className="text-sm font-semibold text-gray-900 mb-3">Навыки</h2>
      ...
      {/* ── Проекты ── */}
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <h2 className="text-sm font-semibold text-gray-900 mb-3">Проекты</h2>
        {emp.allocations.length > 0 ? (
          ...
          <p className="text-sm text-gray-500">Нет назначенных проектов.</p>
        )}
      </div>

      {/* ── Мини-таймлайн ── */}
      <div className="bg-white border border-gray-200 rounded-xl p-6">
        <h2 className="text-sm font-semibold text-gray-900 mb-3">Таймлайн (Q1 2026)</h2>
        <div className="relative">
          <div className="flex mb-1">
            {monthLabels.map((m) => (
              <div key={m} className="flex-1 text-xs text-gray-500 text-center">
                {m}
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {/* ── Достижения ── */}
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          <h2 className="text-sm font-semibold text-gray-900 mb-3">Достижения</h2>
        ...
        {/* ── Отпуск ── */}
        <div className="bg-white border border-gray-200 rounded-xl p-6">
          <h2 className="text-sm font-semibold text-gray-900 mb-3">Отпуск</h2>
          {emp.vacation.length > 0 ? (
            ...
            <p className="text-sm text-gray-500">Нет запланированных отпусков.</p>
          )}
        </div>
      </div>
    </div>
