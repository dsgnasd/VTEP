...
const getCompletionItems = (hasAvatar) => [
  { label: 'Фото профиля', points: 15, done: !!hasAvatar, action: 'avatar' },
...
function ProfileTabsPanel({ activeTab, onTabChange, hasAvatar, onAvatarUpload }) {
  return (
    <Card padding="none" className="lg:col-span-3 overflow-hidden">
      ...
      <div className="px-1.5 pt-4 pb-2.5 sm:px-2 sm:pt-5 sm:pb-3">
        {activeTab === 'Обзор' && (
          <OverviewTab
...
function ExperienceTab() {
  ...
  const deleteBtn = (onClick) => (
    <button onClick={onClick} className="p-1.5 rounded-md text-gray-500 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/20 transition" title="Удалить" aria-label="Удалить">
      <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
